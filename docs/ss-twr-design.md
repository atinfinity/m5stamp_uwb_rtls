...
- タイミング初期値: `responseTxDelayUus`(定数 `kSsResponseTxDelayUus`)3000 µs、RX タイムアウト(`kSsRxTimeoutUus`)4500 µs(公式サンプル準拠。確定値は `rtls_common.h` の `kSs*` 定数を正とする)。

## 4. DS-TWR からの切替判断

| # | 前提条件(すべて実測で確認) | 検証方法 |
|---|---|---|
| ① | ライブラリの SS-TWR がクロックオフセット補償を行い、静的誤差が **σ ≤ 15 cm・バイアス ≤ 10 cm** に収まる | §6 の 1. の静的試験を DS-TWR と並べて実施 |
| ② | SS-TWR の 1 交換所要時間が DS-TWR 比で **30% 以上短い** | §6 の 2. タイミング実測 |
