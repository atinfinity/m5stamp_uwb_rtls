...
- **時刻同期**: ナノ秒精度は不要(UWB の測距自体は同期不要)。スロット境界を守れる **±10 ms 程度**で十分なので、タグは Wi-Fi 経由の **SNTP 同期**を使い、`(現在時刻 ms) mod 500` で自スロットを判定する。ESP32 の SNTP は LAN 内なら数 ms 精度が出るため成立する。
...
→ **A案の詳細設計は [server-design.md](server-design.md) を参照。**
